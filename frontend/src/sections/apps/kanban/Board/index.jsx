...
export default function Board() {
  ...
  const onDragEnd = (result) => {
    ...
    if (type === 'column') {
      const newColumnsOrder = Array.from(backlogs?.columnsOrder);
    ...
    const sourceColumn = backlogs?.columns.filter((item) => item.id === source.droppableId)[0];

    // find dropped item's column
    const destinationColumn = backlogs?.columns.filter((item) => item.id === destination.droppableId)[0];
    ...
    if (sourceColumn === destinationColumn) {
      ...
      newColumn = backlogs?.columns.map((column) => {
