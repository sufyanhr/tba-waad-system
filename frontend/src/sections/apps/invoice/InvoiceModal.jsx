...
export default function InvoiceModal({ isOpen, setIsOpen, invoiceInfo, items, onAddNextInvoice, invoiceMaster }) {
  ...
  return (
    <Dialog
      ...
      maxWidth="md"
      slotProps={{
        paper: { sx: { p: 0, minWidth: { xl: 1200, sm: 'calc(100% - 20%)' } } },
        backdrop: { sx: { opacity: '0.5 !important' } }
      }}
    >
      <Box sx={{ pb: 2.5 }}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ justifyContent: 'space-between' }}>
              <Box sx={{ pt: 2.5 }}>
                <Stack direction="row" sx={{ gap: 2 }}>
                  <Logo to="#" /> <Chip label={invoiceInfo && invoiceInfo.status} variant="light" color="success" />
                </Stack>
                ...
                {items.map((item) => (
                  <TableRow key={item.name} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    ...
                    <TableCell align="right">
                      {invoiceMaster && invoiceMaster.country?.prefix + '' + Number(item.price).toFixed(2)}
                    </TableCell>
                    <TableCell align="right">
                      {invoiceMaster && invoiceMaster.country?.prefix + '' + Number(item.price * item.qty).toFixed(2)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
                ...
                <Stack sx={{ gap: 2 }}>
                  ...
                  <Stack direction="row" sx={{ justifyContent: 'space-between' }}>
                    ...
                    <Typography variant="h6">
                      {total % 1 === 0 ? invoiceMaster.country?.prefix + '' + total : invoiceMaster.country?.prefix + '' + total.toFixed(2)}
                    </Typography>
                  </Stack>
                </Stack>
              </Grid>
        ...
        <Stack direction="row" sx={{ gap: 2, justifyContent: 'flex-end', p: 2.5 }}>
          ...
          <PDFDownloadLink
            document={<ExportPDFView list={invoiceInfo} themeMode={scheme} currentPalette={currentPalette} />}
