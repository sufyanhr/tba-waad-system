...
function StyledTreeItem({ bgColor, color, labelIcon, labelInfo, labelText, ...other }) {
  return (
    <StyledTreeItemRoot
      label={
        <Stack direction="row" sx={{ alignItems: 'center', p: 0.5, pr: 0 }}>
...
export default function GmailTreeView() {
  ...
  const CaretDownIcon = ({ ownerState, ...props }) => <CaretDownFilled {...props} />;
  const CaretRightIcon = ({ ownerState, ...props }) => <CaretRightFilled {...props} />;

  const gmailTreeviewCodeString = `// GmailTreeView.tsx
<SimpleTreeView
  aria-label="gmail"
  defaultExpandedItems={['3']}
  sslots={{ collapseIcon: CaretDownIcon, expandIcon: CaretRightIcon, endIcon: EndIcon }}
  ...
  return (
    <MainCard title="Gmail Clone" codeString={gmailTreeviewCodeString}>
      <SimpleTreeView
        aria-label="gmail"
        defaultExpandedItems={['3']}
        slots={{ collapseIcon: CaretDownIcon, expandIcon: CaretRightIcon, endIcon: EndIcon }}
        sx={{ height: 400, flexGrow: 1, overflowY: 'auto' }}
      >
        <StyledTreeItem itemId="1" labelText="All Mail" labelIcon={<MailFilled />} />
        <StyledTreeItem itemId="2" labelText="Trash" labelIcon={<DeleteFilled />} />
