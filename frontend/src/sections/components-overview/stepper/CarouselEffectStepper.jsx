...
function SampleNextArrow({ className, style, onClick }) {
  return (
    <Box className={className} sx={{ ...style, display: 'block', top: '87.5%', right: 64 }}>
      <Button
        onClick={onClick}
        endIcon={<RightOutlined size={14} />}
        sx={(theme) => ({
          my: 2,
          mx: 1,
          ...(theme.direction === ThemeDirection.RTL && { mr: 4, '& .MuiButton-icon.MuiButton-endIcon': { mr: 0.5 } })
        })}
        size="small"
      >
        Next
...
function SamplePrevArrow({ className, style, onClick }) {
  return (
    <Box className={className} sx={{ ...style, display: 'block', top: '87.5%', left: 0 }}>
      <Button
        onClick={onClick}
        startIcon={<LeftOutlined size={14} />}
        sx={(theme) => ({
          my: 2,
          mx: 1,
          ...(theme.direction === ThemeDirection.RTL && { mr: -10, '& .MuiButton-icon.MuiButton-startIcon': { ml: 0.5 } })
        })}
        size="small"
      >
        Back
