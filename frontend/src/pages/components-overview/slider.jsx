...
export default function ComponentSlider() {
  ...
  const iconsSliderCodeString = `<Stack direction="row" sx={{ mb: 1, gap:2, alignItems:"center"}} >
  <AudioMutedOutlined style={{ color: volume <= 25 ? 'inherit' : theme.vars.palette.text.secondary }} />
  <Slider aria-label="Volume" value={volume} onChange={handleVolumeChange} />
  <SoundOutlined style={{ color: volume > 25 ? 'inherit' : theme.vars.palette.text.secondary }} />
</Stack>`;
...
/>`;
  ...
  const verticalSliderCodeString = `<Stack sx={{ height: 300, gap: 1 }}  direction="row">
