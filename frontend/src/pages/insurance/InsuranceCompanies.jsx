...
import insuranceApi from 'api/insuranceCompaniesApi';
...
export default function InsuranceCompanies() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await insuranceApi.list(params);
  ...
  const handleSave = async (values) => {
    try {
      setLoading(true);
      if (editing?.id) {
        await insuranceApi.updateInsurance(editing.id, values);
        toast.success('Company updated');
      } else {
        await insuranceApi.createInsurance(values);
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await insuranceApi.deleteInsurance(deletingId);
    ...
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <MainCard>
          <Grid container alignItems="center" spacing={2} sx={{ mb:2 }}>
            <Grid item xs>
              <TextField size="small" placeholder="Search companies..." value={search} onChange={(e)=>{ setSearch(e.target.value); setPage(0); }} />
            </Grid>
            <Grid item>
              {canManage && (
                <Button variant="contained" onClick={openCreate}>+ Add Company</Button>
              )}
            </Grid>
          </Grid>
          ...
          {loading ? (
            ...
            <EmptyState title="No Companies" description="No insurance companies found." actionLabel="Retry" onAction={fetchData} />
          ) : (
      ...
      <Drawer anchor="right" open={drawerOpen} onClose={closeDrawer}>
        <div style={{ width: 480, padding: 16 }}>
          <h3>{editing?.id ? 'Edit Company' : 'Add Company'}</h3>
          <CompanyForm initial={editing} onCancel={closeDrawer} onSave={handleSave} />
        </div>
      </Drawer>
